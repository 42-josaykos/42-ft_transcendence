...
export const useChannelStore = defineStore("channel", () => {
  ...
  const updateOwner = (userID: number | undefined) => {
    if (userID != undefined) {
      Get(
        `http://${HOST}:${API_PORT}/users/search?id=${userID}&ownerChannels`
      ).then((res) => {
        if (res.status == 200) {
          const ownerChannels = res.data[0].ownerChannels;
          for (const chan of allChannels.value) {
            const index = ownerChannels.findIndex(
              (el: Channel) => el.id === chan.id
            );
            if (index != -1) {
              chan.isOwner = true;
            } else {
    ...
    if (channelUpdate.value != undefined) {
      if (users.value != undefined) {
        for (const user of users.value) {
          if (
            channelUpdate.value.members.findIndex((el) => el.id == user.id) ==
            -1
          ) {
            return false;
          }
  ...
  const isAdmin = (
    channel_item: Channel | undefined,
  ...
  const secToTime = (sec: number, isBan: boolean, channelId: number) => {
    ...
    if (isBan) {
      const index = timerIntervalBan.value.findIndex(
        (el: any) => el.channelId == channelId
      );
      if (index != -1) {
        timerIntervalBan.value[index].timeLeft =
          strH + hours + ":" + strM + minutes + ":" + strS + seconds;
      }
    } else {
      const index = timerIntervalMute.value.findIndex(
        (el: any) => el.channelId == channelId
      );
      if (index != -1) {
        timerIntervalMute.value[index].timeLeft =
          strH + hours + ":" + strM + minutes + ":" + strS + seconds;
      }
    }
  };
  ...
  const timer = (dateEnd: any, channelItem: Channel, isBan: boolean) => {
    ...
    if (dateNow.getTime() < dateEnd.getTime()) {
      ...
      if (isBan) {
        const index = timerIntervalBan.value.findIndex(
          (el: any) => el.channelId == channelItem.id
        );
        if (index != -1) {
  ...
  const handleBanMute = (channelItem: Channel, isBan: boolean) => {
    ...
    const index = channel_item.findIndex(
      (el: any) => el.user.id == loggedUser.value?.id
    );
    const indexBan = timerIntervalBan.value.findIndex(
      (el: any) => el.channelId == channelItem.id
    );
    const indexMute = timerIntervalMute.value.findIndex(
      (el: any) => el.channelId == channelItem.id
    );
    if (index != undefined) {
  ...
  const updateBanMute = (data: any) => {
    ...
    if (channelsBan != undefined) {
      for (const ban of channelsBan) {
        const channelItem = allChannels.value.find(
          (el: Channel) => el.id === ban.channel.id
        );
    ...
    if (channelsMute != undefined) {
      for (const mute of channelsMute) {
        const channelItem = allChannels.value.find(
          (el: Channel) => el.id === mute.channel.id
        );
        if (channelItem != undefined) {
