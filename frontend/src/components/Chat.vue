...
onMounted(async () => {
  Get("/channels/search?&password").then(
    (res) => (allChannels.value = res.data)
  );
  Get(
    "/users/search?id=" +
      ...
      if (res.status == 201) {
        ...
        socket.emit(
          "msgToServer",
          {
            ...
            data: textDirectMsg.value,
          },
          loggedUser.value
        );
        textDirectMsg.value = "";
        Get("/channels/search?id=" + res.data.id.toString() + "&members").then(
          (res) => (usersMembers.value = res.data[0].members)
        );
      }
    });
  } else {
...
const joinChannel = () => {
  ...
  if (channelJoin.value != undefined) {
    ...
    Patch(
      baseUrlChannel + "/" + channelJoin.value?.id.toString(),
      updateChannel
    ).then((res) => {
      channelStore.joinChannel(res.data);
...
const leaveChannelIfNotOwner = (channel_item: Channel) => {
  ...
  Get(
    ...
    if (loggedUser.value != null) {
      ...
      Patch(
        baseUrlChannel + "/" + channel_item.id.toString(),
        updateChannel
      ).then((res) => {
        ...
        channel.value =
          channel.value?.id === channel_item.id ? undefined : channel.value;
        messages.value =
          channel.value?.id === channel_item.id ? [] : messages.value;
        channelStore.updateMember();
      });
    }
  });
    ...
    if (channelLeave.value !== undefined) {
      if (
        channelStore.isAdmin(
          channelLeave.value,
          newOwner.value != undefined ? newOwner.value : -1
        ) == true
      ) {
        updateChannel = {
      ...
      Patch(
        baseUrlChannel + "/" + channelLeave.value.id.toString(),
        updateChannel
      ).then((res) => {
        ...
        channelStore.updateMember();
        channelStore.updateOwner(
          loggedUser.value != null ? loggedUser.value.id : -1
        );
...
socket.on("updateMemberChannelToClient", (channelID: number) => {
  if (channel.value?.id === channelID) {
    Get(
      "/channels/search?id=" +
        channelID.toString() +
        "&owner&members&admins&mutes&bans"
    ).then((res) => {
      channel.value = res.data[0];
      usersMembers.value = res.data[0].members;
    });
...
const deleteChannel = () => {
  Delete(baseUrlChannel + "/" + channelLeave.value?.id.toString()).then(
    (res) => {
      if (res.status == 200) {
...
const updateChannel = () => {
  if (channelUpdate.value !== undefined) {
    ...
    Patch(baseUrlChannel + "/" + input.value.channel_id, updateChannel).then(
      (res) => {
        if (res.status == 200) {
          socket.emit("updateChannelToServer", res.data);
        }
...
socket.on("updateChannelToClient", (updateChannel: Channel) => {
  if (loggedUser.value != null) {
    ...
    if (channel.value?.id === updateChannel.id) {
      Get(
        "/channels/search?id=" + channel.value.id.toString() + "&members"
      ).then((res) => (usersMembers.value = res.data[0].members));
  ...
  <div class="container-fluid chat">
    <div class="chatMenu">
      <div class="chatMenuWrapper">
        ...
        <button
          @click="channelsJoin = true"
          type="button"
          class="btn btn-secondary send"
        >
          Channels
        </button>
        ...
        <button
          @click="
            (channelsJoin = false),
              channelStore.updateMember(),
              channelStore.updateOwner(loggedUser != null ? loggedUser.id : -1)
          "
          type="button"
          class="btn btn-secondary send"
        >
          ...
          <div v-if="channels">
            ...
            >
              ...
              <button
                ...
                type="button"
                class="btn btn-secondary btn-channel"
              >
                {{ searchName(item) }}
              </button>
            </ul>
            ...
            >
              <div v-if="item.isMember">
                ...
                <button
                  ...
                  class="btn btn-secondary btn-channel"
                >
                  <span v-if="item.isPrivate == true" class="badge bg-success"
                    >Private</span
                  >
                  <span
                    v-else-if="item.password != null"
                    class="badge bg-warning"
                    >Pass : {{ item.password }}</span
                  >
                ...
                <div v-if="item.isOwner">
                  ...
                  <button
                    type="button"
                    class="btn btn-danger btn-channel btn-sm"
                    @click="
                      Get(
                        'channels/search?id=' +
                          item.id.toString() +
                          '&admins&mutes&members'
                      ).then((res) => ([channelLeave] = res.data))
                    "
                    data-bs-toggle="modal"
                    data-bs-target="#leaveChannel"
                  >
                    Leave Owner
                  </button>
                ...
                <div v-else>
                  ...
                  <button
                    type="button"
                    class="btn btn-danger btn-channel btn-sm"
                    @click="leaveChannelIfNotOwner(item)"
                  >
                    Leave
                  </button>
                </div>
              ...
              <div v-else>
                ...
                <div v-if="item.isPrivate == false">
                  ...
                  <button
                    type="button"
                    class="btn btn-primary btn-channel btn-sm"
                    @click="channelJoin = item"
                    data-bs-toggle="modal"
                    data-bs-target="#joinChannel"
                  >
                    Join
                  </button>
                </div>
          ...
          <div v-if="channelsInvite">
            ...
            >
              ...
              <button
                type="button"
                class="btn btn-primary btn-channel btn-sm"
                @click="(channelJoin = item), acceptInviteChannel()"
              >
                Join
              </button>
              <button
                type="button"
                class="btn btn-danger btn-channel btn-sm"
                @click="(channelJoin = item), refuseInviteChannel()"
              >
                Refuse
              </button>
            </ul>
    ...
    <div class="chatBox">
      <div class="chatBoxWrapper">
        ...
        <div v-if="channel != undefined">
          ...
          <form
            @submit.prevent.trim.lazy="handleSubmitNewMessage(channel?.id)"
            method="POST"
            class="form"
          >
                ...
                <div v-if="channelStore.isBan(channel, user.id) == false">
                  <a class="list-group-item list-group-item-action">
                    ...
                    <div v-if="channelStore.isAdmin(channel, user.id)">
                      ...
                      <div
                        v-if="
                          channelStore.isOwner(channel, loggedUser?.id) &&
                          loggedUser?.id != user.id
                        "
                      >
                        <button
                          @click="
                            updateChan({ removeAdmins: [{ id: user.id }] })
                          "
                          type="button"
                          class="btn btn-danger btn-channel btn-sm"
                        >
                    ...
                    <div v-else-if="channelStore.isMember(channel, user.id)">
                      ...
                      <div v-if="channelStore.isOwner(channel, loggedUser?.id)">
                        <button
                          @click="updateChan({ addAdmins: [{ id: user.id }] })"
                          type="button"
                          class="btn btn-success btn-channel btn-sm"
                        >
                          Add admin
                    ...
                    >
                      <button
                        @click="userDirectMessage = user"
                        type="button"
                        class="btn btn-info btn-channel btn-sm"
                        data-bs-toggle="modal"
                    ...
                    <div
                      v-if="
                        loggedUser?.id != user.id &&
                        !channelStore.isAdmin(channel, user.id) &&
                        !channelStore.isMute(channel, user.id)
                      "
                    >
                      <button
                        @click="updateChan({ addMutes: [{ id: user.id }] })"
                        type="button"
                        class="btn btn-warning btn-channel btn-sm"
                      >
                        Mute
                      </button>
                    </div>
                    ...
                    <div
                      v-if="
                        loggedUser?.id != user.id &&
                        !channelStore.isAdmin(channel, user.id) &&
                        !channelStore.isBan(channel, user.id)
                      "
                    >
                      <button
                        @click="updateChan({ addBans: [{ id: user.id }] })"
                        type="button"
                        class="btn btn-warning btn-channel btn-sm"
                      >
                        Ban
                    ...
                    >
                      <button
                        @click="updateChan({ removeMutes: [{ id: user.id }] })"
                        type="button"
                        class="btn btn-warning btn-channel btn-sm"
                      >
                        Remove mute
                ...
                <div v-else>
                  *** Users Bans ***
                  <a class="list-group-item list-group-item-action">
                    ...
                    >
                      <button
                        @click="updateChan({ removeBans: [{ id: user.id }] })"
                        type="button"
                        class="btn btn-warning btn-channel btn-sm"
                      >
                        Remove ban
    ...
    >
      <div class="modal-dialog">
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title" id="staticBackdropLabel">
              Update : {{ input.update_channel_name }} =>
              {{ channelTypeUpdate }}
            </h5>
            <button
              type="button"
          ...
          <div class="modal-body">
            <div class="form form-new-channel">
              <label for="name">Channel name:</label>
              <input
                v-model="input.update_channel_name"
                type="text"
                class="input"
              />
            </div>
            ...
            <div>
              Channel :
              {{
                channelUpdate?.isPrivate
                  ? "Private"
                  : channelUpdate?.password != null
                  ? "Proteted"
                  : "Public"
              }}
            </div>
                ...
                <div class="scroller">
                  <div class="list-group" v-for="user in users" :key="user.id">
                    <div
                      v-if="
                        user.id != loggedUser?.id &&
                        channelStore.isMember(channelUpdate, user.id) ==
                          false &&
    ...
    >
      <div class="modal-dialog">
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title" id="staticBackdropLabel">
              Leave channel : {{ channelLeave?.name }}
            </h5>
            <button
              @click="newOwner = -1"
              type="button"
                ...
                >
                  <div v-if="item.id != loggedUser?.id">
                    <a class="list-group-item list-group-item-action">
                      {{ item.username }} =>
                      <button
                        @click="newOwner = item.id"
                        type="button"
                        class="btn btn-danger btn-channel"
                        data-bs-toggle="modal"
                        data-bs-target="#validateNewOwner"
                      >
                        New Owner
                      </button>
                    </a>
