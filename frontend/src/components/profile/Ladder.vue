...
<template>
  ...
  <table style="width: 90%; table-layout: fixed; margin-left: 5%">
    ...
    <tbody>
      <tr v-for="(player, index) in players?.slice(0 , 20)"> <!-- Print top 20 of the leaderboard -->
        <th
          v-bind:class="{ 'first' : index + 1 === 1 ,
                          'second' : index + 1 === 2 ,
                          'third' : index + 1 === 3}"
        >
          {{ index + 1 }}
        </th>
        <td><button class="user_btn" @click="userClick = player.user">{{ player.user.username }}</button></td>
        <td>{{ player.ratio }} %</td>
      </tr>
    </tbody>
  </table>
