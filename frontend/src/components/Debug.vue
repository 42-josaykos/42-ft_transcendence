...
async function getUserByLogin() {
  try {
    user.value = await axios.get(url + search_input.value)
  } catch (e: any) {
...
async function createUser() {
  const new_user = { login: create_input.value }
  axios.post(url, new_user).then(() => {
    getAllUsers()
  }).catch(e =>
    e.response.data
  )
...
async function updateUser(id: string, updated_data: any) {
  axios.patch(url + id, updated_data).then(() => {
    getAllUsers()
  }).catch(e =>
    e.response.data
  )
...
async function deleteUser(e: any) {
  axios.delete(url + e.target.parentElement.id).then(() => {
    getAllUsers()
  }).catch(e =>
    e.response.data
  )
...
<template>
  ...
  <form @submit.prevent.trim.lazy="getUserByLogin" method="GET">
    ...
    <input v-model="search_input" name="name" type="text" />
